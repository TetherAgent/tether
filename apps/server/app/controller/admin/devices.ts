...
export default class AdminDevicesController extends Controller {
  public async index(): Promise<void> {
    ...
    const data = await ctx.service.admin.devices.listAdminDevices(page, limit);
    ctx.success(data);
  }
  ...
  public async revoke(): Promise<void> {
    ...
    await ctx.service.admin.devices.revokeAdminDevice(id, identity.adminUserId ?? '', identity.accountId, identity.workspaceId);
    ctx.success({ ok: true });
  }
}
