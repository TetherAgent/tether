...
再收口本地体验、tmux cleanup、完整 device-token auth、retention、supervisor 和
security tests。该重排不改变长期 Phase 3/4 方向；它只是把原本后置的 Relay 可达性
提前成个人 MVP。
...
device token 认证。当前 v0.3 的 Personal Relay MVP 可先用 owner-configured relay
link secret 打通个人自托管链路，但 Relay 只能转发认证后的协议 frame，不能执行命令、
不能接受任意 provider command/args/env、不能持久化终端明文；完整 device-token
认证随后在 Authentication phase 收口。
...
- `tether run <provider> --no-attach` 可以启动多个 session，但每个 PTY session
  仍由启动它的 CLI/Gateway 进程持有。
- 当前 `tether gateway` 是前台常驻空 Gateway，能托管 Web/API 和标记历史 lost
  session，但还不是完整 supervisor；它还不能接收“创建 session”API 后统一持有
  所有后续 session。
- 如果要同时跑多个 `--no-attach` session，当前建议为每个启动进程使用不同端口，
  例如 `--port 4789`、`--port 4790`。完整目标是后续改成一个常驻 Gateway
  统一创建和持有所有 session，CLI 只发控制请求。
