...
收尾 Phase 2（PTY-backed event stream），让"远程访问"从能跑变成可信赖：detach
快捷键到位、TUI 行为通过专项验证、device token 真正生效、Gateway 作为 supervisor
稳得住、tmux fallback 下线。

里程碑出口：用户可以从手机 / Web 安全访问运行在 Mac 上的 Gateway，本地终端体验
不退步，Phase 2 验收清单全绿。
...
### Active（v0.3 — Remote Access 范围）

**P0 — 本地体验硬指标（Phase 2A/2B 验收 ❌ 项）**
...
- [ ] **AUTH-01**: input / resize / stop / claim-control 全部接入 device token auth
- [ ] **AUTH-02**: 完整 pairing 流程（一次性 code + token hash 落库）
...
- **Cloudflare Tunnel / Tailscale 集成的工程化适配** — Phase 1.5 完整版推到下个 milestone；v0.3 只保证 LAN + device token 安全
- **自建 Relay MVP** — Phase 1.5 后段；v0.3 期间靠 Tunnel/Tailscale 即可
...
| Phase 2 收尾叫 v0.3 — Remote Access | device token + supervisor 完成后才能放心暴露给手机；用版本号体现远程访问门槛 | — Pending（本 milestone 出口） |
| Phase 1.5 完整版（tunnel/relay）推到 v0.4+ | v0.3 范围已经足够大；先把"本地+LAN 安全暴露"做扎实再做远程穿透 | — Pending |
