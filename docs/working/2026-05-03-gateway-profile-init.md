...
```bash
pnpm tether gateway local
pnpm tether gateway direct
pnpm tether gateway relay
```
...
| `gateway stop` | 停止后台服务 |
| `gateway local/direct/relay` | 按指定 profile 前台运行 |
...
`gateway start` 的选择提示必须解释三个模式，帮助用户选对：

```text
tether gateway local  - 开发人员本机调试：只监听 127.0.0.1，不给局域网访问
tether gateway direct - 局域网直连：监听 0.0.0.0，浏览器直接连本机 Gateway，不走 Relay
tether gateway relay  - 公网远程：本机 Gateway 主动连接 Relay，适合不在同一局域网时使用
```
...
```bash
TETHER_RELAY_SECRET=xxx pnpm tether gateway relay
```
...
- 新增 `gateway init`。
- 新增 `gateway local` / `gateway direct` / `gateway relay`。
- `gateway` 无子命令时按 `defaultProfile` 启动。
...
- `gateway direct` 前台启动时，如果 `auth.json` 缺失或过期，是直接提示登录，还是自动进入登录 prompt。
