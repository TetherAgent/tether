...
长期规范是 Direct 和 Relay 都对齐 Gateway-owned subscribe / replay / live 模型：
Web 只表达订阅意图，Gateway 负责 resize、paged replay、`replay.done` 和 live cursor；
Relay 只转发订阅、事件和 `replay.done`。

- 当前 Direct 模式仍由 Web 直接调用 Gateway HTTP `/events`，前端负责按 cursor 分页拉取完整历史。
  后续主路径应改为 Direct WS `/stream` 由 Gateway 分页 replay。
- 当前 Relay 模式由 Gateway relay-client 处理 `client.subscribe`，Gateway 必须分页发送
