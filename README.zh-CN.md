...
`tether run codex`、`tether run claude`、`tether run opencode` 会跑在 Gateway 托管的
...
以前：claude
现在：tether run claude
```

电脑上敲 `tether run codex` 或 `tether run claude`，Tether 接管这条 agent，打印一个 URL。手机扫开，就是同一个会话现场——电脑里在跑什么，手机上一字不落；手机上敲一行字，agent 立刻收到。代码继续在你机器上执行，凭证从不离开本地。
...
- `tether run codex` / `tether run claude`：一行命令把 agent 包进托管会话
...
# 示例：
tether run codex --title "修复 auth bug"
tether run codex --model o3                    # 透传参数给 codex 本身
tether run shell --title "Terminal"            # 启动真实登录 shell
...
远程访问由登录后的 Gateway / Relay 链路负责，不再通过 provider 启动命令传 `--host`。
...
- 会话明文默认不上云。
- 手机能请求的本机动作是白名单制的：打开桌面 Web UI、attach 既有 session、把消息送给 agent 或显式创建的 `shell` session——仅此而已。
- 手机**永远不能**让 Gateway 执行任意 shell 命令；`shell` 只是一种白名单 provider，固定启动登录 shell。
...
- **不是通用远程 shell**。Gateway 不接受任意命令执行；`shell` 只是显式白名单 provider。
...
- **暴露必须显式**：要在局域网共享，必须通过 Gateway 配置显式开放，不会因为开关位置巧合而泄露。
- **写操作要凭证**：Phase 2.5 起，客户端任何写操作都需要 device token。
- **客户端只能接入白名单 session**：手机/Web 可以向既有 agent 或 `shell` session 发输入，但无法获得任意命令执行能力。
